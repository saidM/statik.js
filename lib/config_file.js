...
const read = () => {
  return new Promise((resolve, reject) => {
    fs.readFile(filename, 'utf8', (err, data) => {
      ...
      axios.get(`http://www.statik.run/credentials/${subdomain}/${secretKey}`)
      .then(() => resolve({subdomain, secretKey}))
      .catch(() => reject('INVALID_CONFIG_FILE'))
    })
  })
}
...
const create = (subdomain = Date.now(), secretKey = secret) => {
  return new Promise((resolve, reject) => {
    ...
    fs.writeFile(filename, content, 'utf8', (err, data) => {
      ...
      axios.post(`http://www.statik.run/sites/${subdomain}/${secretKey}`)
      .then(res => resolve({subdomain, secretKey}))
